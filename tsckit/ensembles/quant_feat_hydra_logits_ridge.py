...
class QuantFeatHydraLogitsRidge:
    """Stacked ensemble: Quant features + Hydra OOF logits → Ridge classifier.

    Architecture:
    - Level 1: Quant features (quantile-based intervals) + Hydra OOF logits (CV-based)
    - Level 2: RidgeClassifierCV meta-learner combines both representations

    Differs from QuantFeatHydraLogitsStack only in meta-learner choice (Ridge vs ExtraTrees).
    """
    ...
    def fit(self, dataset: Dataset) -> 'QuantFeatHydraLogitsRidge':
        ...
        # Fit Ridge classifier on stacked features
